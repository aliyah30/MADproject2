...
exports.notifyOnPackingListChange = exports.notifyOnItineraryChange = exports.optimizeItinerary = void 0;
...
function estimateTravelMinutes(fromLocation, toLocation) {
...
function scoreActivity(activity, prevLocation, cumulativeCost, budget, visitHour) {
    var _a, _b;
    const reasons = [];
    let penalty = 0;
    // Distance penalty
    ...
    if (hours !== null) {
        if (visitMinutes < hours.open) {
            ...
            reasons.push(`arrives ${Math.round(waitMin / 60 * 10) / 10}h before opening at ` +
                `${activity.openHours.split(/[-–]/)[0].trim()}`);
        }
        else if (visitMinutes > hours.close) {
            penalty += 40;
            reasons.push(`already closed at this time (closes ${(_b = (_a = activity.openHours.split(/[-–]/)[1]) === null || _a === void 0 ? void 0 : _a.trim()) !== null && _b !== void 0 ? _b : ""})`);
        }
        else {
    ...
    if (newCumulative > budget) {
        ...
        reasons.push(`adds $${activity.cost.toFixed(0)} — cumulative $${newCumulative.toFixed(0)} exceeds $${budget.toFixed(0)} budget`);
    }
    else {
...
function buildMoveReason(originalIndex, newIndex, scoreReasons) {
    var _a, _b, _c;
    if (originalIndex === newIndex) {
        return `Kept in position ${newIndex + 1} — already optimally placed. ${(_a = scoreReasons[0]) !== null && _a !== void 0 ? _a : ""}`;
    }
    const direction = newIndex < originalIndex ? "earlier" : "later";
    const primary = (_b = scoreReasons[0]) !== null && _b !== void 0 ? _b : "";
    const secondary = (_c = scoreReasons[1]) !== null && _c !== void 0 ? _c : "";
    return (`Moved ${direction} to position ${newIndex + 1}: ${primary}` +
        (secondary ? `; ${secondary}` : "") +
        ".");
}
// ─────────────────────────────────────────────────────────────────────────────
...
exports.optimizeItinerary = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "You must be signed in to optimize an itinerary.");
    }
    const { tripId, budget, activities } = data;
    if (!tripId || !activities || activities.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "tripId and a non-empty activities array are required.");
    }
    ...
    if (!((_a = tripData.memberUids) === null || _a === void 0 ? void 0 : _a.includes(context.auth.uid))) {
        throw new functions.https.HttpsError("permission-denied", "You are not a member of this trip.");
    }
    const START_HOUR = 9;
    ...
    while (remaining.length > 0) {
        ...
        for (let i = 0; i < remaining.length; i++) {
            ...
            const { score, reasons } = scoreActivity(candidate, currentLocation, cumulativeCost, budget, visitHour);
            if (score > bestScore) {
                bestScore = score;
                bestIdx = i;
                bestReasons = reasons;
        ...
        const originalIndex = activities.findIndex((a) => a.activityId === chosen.activityId);
...
// ─────────────────────────────────────────────────────────────────────────────
exports.notifyOnItineraryChange = functions.firestore
    .document("activities/{activityId}")
    .onWrite(async (change) => {
    const isDelete = !change.after.exists;
    const isCreate = !change.before.exists;
    const activityData = (change.after.exists ? change.after.data() : change.before.data());
    if (!activityData)
        return;
    ...
    if (!tripDoc.exists)
        return;
    const tripData = tripDoc.data();
    const body = isDelete ? `"${name}" was removed from the itinerary` :
        isCreate ? `"${name}" was added to the itinerary` :
            `"${name}" was updated in the itinerary`;
    const memberDocs = await Promise.all(tripData.memberUids.map((uid) => db.collection("users").doc(uid).get()));
    const tokens = [];
    ...
    if (staleTokens.length > 0) {
        ...
        for (const doc of memberDocs) {
            if (!doc.exists)
                continue;
            const userData = doc.data();
            const userStale = staleTokens.filter((t) => { var _a; return (_a = userData.fcmTokens) === null || _a === void 0 ? void 0 : _a.includes(t); });
...
// ─────────────────────────────────────────────────────────────────────────────
exports.notifyOnPackingListChange = functions.firestore
    .document("packingList/{tripId}")
    .onUpdate(async (change, context) => {
    const tripId = context.params.tripId;
    const before = change.before.data();
    const after = change.after.data();
    let changedItemName = null;
